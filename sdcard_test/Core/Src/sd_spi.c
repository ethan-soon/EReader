...
SD_Status SD_SPI_Init(void) {
    ...
    /* Card init must run at <=400 kHz, but the data phase can go much faster.
     * Until now everything ran at ~164 kHz (42 MHz APB1 / 256), which makes the
     * f_getfree() FAT scan take minutes. Bump to 42 MHz / 8 = ~5.25 MHz, which
     * is safe over breadboard wiring. Drop to _16 (~2.6 MHz) if reads get flaky. */
